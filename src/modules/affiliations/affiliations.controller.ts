...
export class AffiliationsController {
    ...
    async findAllByUserNotPaginate(
        ...
        const { productId, organizationReceivedId, organizationSellerId } =
            query;
    ...
    async createOne(
        ...
        if (findOneAffiliation)
            throw new HttpException(
                `This product affiliate to this user please change`,
                HttpStatus.NOT_FOUND
            );
    ...
    async updateOne(
        ...
        const { email, percent, description, expiredAt, productId } = body;
        const findOneAffiliation = await this.affiliationsService.findOneBy({
            affiliationId,
        });
        if (!findOneAffiliation)
        ...
        await this.affiliationsService.updateOne(
            { affiliationId },
            { email, percent, description, expiredAt, productId }
        );
    ...
    async getOne(@Res() res, @Query() query: GetOneAffiliationDto) {
        const { affiliationId, affiliate } = query;
        const findOneAffiliation = await this.affiliationsService.findOneBy({
            affiliationId,
            code: affiliate,
        });
