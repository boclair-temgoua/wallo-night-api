...
export const userContributorCreateMail = async (options: { contributor }) => {
  ...
  }/confirm-invitation?token=${contributor.token}">View Invitation</a>
  </td>
  </tr>
  </table><br/><br/>
  ...
  This invitation was intended for <a href = "mailto:${contributor.email}">${
    contributor.email
  }</a>. if you were not expecting this invitation, you con ignore this email
  </p>
  ...
  <a href="${configurations.datasite.urlClient}/confirm-invitation?token=${
    contributor.token
  ...
  ${configurations.datasite.urlClient}/confirm-invitation?token=${
    contributor.token
  }
  </a></span></p>
  ...
  await NodeMailServiceAdapter({
    ...
    subject: `${configurations.datasite.name} - Invitation ${contributor?.organization?.name}`,
    html: output,
  });
};
